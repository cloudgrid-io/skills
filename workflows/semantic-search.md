...
collection (a float array) and cosine-ranks in-app. **NO active cron** — a Python
`type: cron` refresh is blocked on **#1585**; the cron service stays commented and
the manager "Refresh now" endpoint does the indexing.
...
Mongo, then search. Give the user the live app URL — the deliverable. To iterate,
...
pgvector until #1545), and refresh is a manual endpoint until the cron unblocks
(#1585).
