...
2. Fetch the template for the Mongo wiring + CRUD shape:
   `grid_get_template("template", "kanban")`. It is a minimal, real Next.js +
...
3. Adapt it (rename the collection, change the columns/fields, adjust the UI).
   **Read the DB from `process.env.DATABASE_MONGODB_URL`** (legacy
   ...
   - (Optional) fetch `grid_get_template("example", "kanban")` for a richer filled
...
- API keys / secrets → `grid secrets set`. Non-secret config → `grid env set`.
