# job-board template — persistent Next.js + Mongo job board
...
- Rename the `jobs` collection in `services/web/app/api/jobs/route.js`
  and `services/web/app/page.js` (e.g. `gigs`, `roles`).
- Change the document fields (salary bands, tags, remote flag, timestamps).
