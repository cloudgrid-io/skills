# event-board template — persistent Next.js + Mongo event board
...
The domain is `events` with fields **title, date, location, description**. The
board lists events sorted by `date` ascending (soonest upcoming event first).
...
- Rename the `events` collection in `services/web/app/api/events/route.js` and
  `services/web/app/page.js` (e.g. `meetups`, `sessions`, `gigs`).
- Change the document fields (add host, RSVP count, category, end time).
