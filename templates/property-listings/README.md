# property-listings template — persistent Next.js + Mongo real-estate site

A minimal but real, deployable property listings site. Listings live in the
grid-shared MongoDB, so they survive refresh and are shared across users/sessions
— unlike a static page whose state is in memory only.

The domain is a `properties` collection with `{ title, price:number, location,
beds:number, description }`. The UI is a responsive card grid with a
location/price filter, a per-property detail view, and an admin add form.
...
- Rename the `properties` collection in
  `services/web/app/api/properties/route.js` and `services/web/app/page.js`
  (e.g. `rentals`, `homes`).
- Change the document fields (add photos, sqft, status).
- Add more filters or routes as the site grows.
