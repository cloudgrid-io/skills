# revenue-dashboard template — persistent Next.js + Mongo revenue dashboard
...
It shows a **total revenue** figure, a **per-product breakdown**, a **recent
sales table**, and a form to **add a sale**. Each sale is a document in a `sales`
collection: `{ product: string, amount: number, at: Date }`.
...
- Rename the `sales` collection in `services/web/app/api/sales/route.js` and
  `services/web/app/page.js` (e.g. `invoices`, `orders`, `payments`).
- Change the document fields (add currency, region, customer, MRR flags).
- Add a chart, date filters, or per-period rollups as the app grows.
