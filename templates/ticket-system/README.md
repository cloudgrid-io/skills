# ticket-system template — persistent Next.js + Mongo support ticket / helpdesk queue
...
- Rename the `tickets` collection in `services/web/app/api/tickets/route.js`
  and `services/web/app/page.js` (e.g. `issues`, `requests`).
- Change the document fields (assignees, tags, SLA due dates, timestamps).
