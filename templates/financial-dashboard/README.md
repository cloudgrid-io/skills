# financial-dashboard template — persistent Next.js + Mongo P&L board

A minimal but real, deployable financial dashboard. Track income and expense
entries by account; the board shows live income / expense / net totals plus a
table with add and delete. Data lives in the grid-shared MongoDB, so it survives
refresh and is shared across users/sessions — unlike a static page whose state is
in memory only.

## Domain

An **entry** is `{ account: string, amount: number, type: "income" | "expense", at: Date }`.
The board derives three **totals** from the entries — income, expense, and net
(income − expense) — and renders them above a table of entries with a per-row
delete and an add form.
...
- Change the document fields (add a category, currency, memo, owner).
