# project-management template — team project & task board (Next.js + Mongo)

A minimal but real, deployable project management app. Manage **projects** and,
per project, a **task board** with three status columns (to do / doing / done)
and an optional assignee per task. Data lives in the grid-shared MongoDB, so it
survives refresh and is shared across users/sessions — unlike a static page whose
...
The home page lists projects; selecting one shows its task board. Deleting a
project cascades and removes its tasks.
...
- Rename the `projects` / `tasks` collections and change the document fields
  (add due dates, priorities, labels).
- Add more statuses to the board (edit the `STATUSES` array in `board.js` and the
  validation in `services/web/app/api/tasks/route.js`).
