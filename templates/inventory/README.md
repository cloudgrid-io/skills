# inventory template — persistent Next.js + Mongo inventory / stock tracker
...
- Rename the `items` collection in `services/web/app/api/items/route.js`
  and `services/web/app/page.js` (e.g. `parts`, `assets`).
- Change the document fields (units, suppliers, cost, reorder thresholds, timestamps).
