# quiz-platform template — persistent Next.js + Mongo quiz app
...
users/sessions — unlike a static page whose state is in memory only. Visitors
take the quiz (pick one option per question, submit for a score) and can add
their own questions through an admin form.

## Domain

A question document is:

```js
{ prompt: string, options: string[], answerIndex: number }
```

`answerIndex` is the 0-based index into `options` that is correct. The score is
computed by comparing each chosen option index against `answerIndex`.
...
- Rename the `questions` collection in `services/web/app/api/questions/route.js`
  and `services/web/app/page.js`.
- Change the document fields (add categories, difficulty, explanations, an owner).
- Add more routes/collections (e.g. persisted quiz attempts and leaderboards).
