# product-catalog template — Next.js + Mongo product catalog
...
static page whose state is in memory only. Visitors browse a product grid and
filter by category; an admin section adds and deletes products. No checkout.
...
- Rename the `products` collection in `services/web/app/api/products/route.js`
  and `services/web/app/page.js` (e.g. `items`, `inventory`, `listings`).
- Change the document fields (add SKU, image URL, tags).
- Add more routes/collections as the catalog grows.
