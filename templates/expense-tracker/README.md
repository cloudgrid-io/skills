# expense-tracker template — persistent Next.js + Mongo expense tracker

A minimal but real, deployable expense tracker. Expenses live in the grid-shared
MongoDB, so they survive refresh and are shared across users/sessions — unlike a
static page whose state is in memory only.

Each expense is `{ description, amount (number), category, date }`. The app lists
expenses newest-first, shows totals by category (plus a grand total), and lets
you add and delete rows.
...
- Rename the `expenses` collection in `services/web/app/api/expenses/route.js`
  and `services/web/app/page.js` if your domain differs.
