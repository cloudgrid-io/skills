# api-dashboard template — Next.js + Mongo API request monitor

A minimal but real, deployable API usage dashboard. Every request is logged to
the grid-shared MongoDB, so the log and its derived metrics survive refresh and
are shared across users/sessions — unlike a static page whose state is in memory.

The domain is a `requests` collection with `{ endpoint, status (number),
ms (number), at }`. The page shows a **metrics header** (request count, average
latency, error rate), a **record form**, and a **recent-requests table** with
per-row delete.
...
Metrics are derived in the client from the current request log:

- **count** — number of logged requests.
- **avg ms** — mean of the `ms` field, rounded.
- **error rate** — share of requests with `status >= 400`, as a percentage.

## Adapt it

- Rename the `requests` collection in `services/web/app/api/requests/route.js`
  and `services/web/app/page.js` (e.g. `events`, `calls`, `hits`).
- Change the document fields (add method, region, user, response size).
- Add more routes/collections, or add filtering/aggregation to the metrics.
