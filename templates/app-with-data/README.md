...
the var). Never hardcode a connection string; never commit a secret. (If you also
declare `cache: true`, the grid injects `CACHE_REDIS_URL` — and the legacy
`REDIS_URL` alias — the same way.)
