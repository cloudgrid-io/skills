# admin-dashboard template — persistent Next.js + Mongo admin panel
...
across users/sessions. Same proven shape as `app-with-data`, adapted to a
metrics header (counts + revenue) over a couple of collections plus a management
table with add/delete.
...
- Rename or replace the `users` / `orders` collections and their routes with
  your own resources.
