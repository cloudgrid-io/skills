...
(pgvector) is **PENDING platform issue #1545**, so embeddings live in Mongo and
similarity is computed in-app until it ships. Because RAG involves choices
...
> (needs + AI Gateway + the pgvector #1545 note), the RAG loop, optional
...
- `cloudgrid.yaml` — active fields: `name`, `services.web` (nextjs, `/`), and
  ...
  commented (PENDING #1545); the optional `vault:` block (auth/payments) is
  commented too.
